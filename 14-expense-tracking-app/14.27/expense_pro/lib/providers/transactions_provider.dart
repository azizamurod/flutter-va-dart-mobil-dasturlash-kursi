...
class TransactionsProvider with ChangeNotifier {
  ...
  void createTransaction(BuildContext context) async {
    ...
    if (_amountController.text == '0') {
      showErrorAlert(context, localizations.enter_amount);
      notifyListeners();
      return;
    }
    ...
    if (_transactionForCreation?.account == null) {
      showErrorAlert(context, localizations.select_account);
      notifyListeners();
      return;
    }
    if (_transactionForCreation?.account2 == null &&
        _transactionForCreation?.type == TransactionTypes.transfer) {
      showErrorAlert(context, localizations.select_second_account);
      notifyListeners();
      return;
    }
    if ((_transactionForCreation!.account!.balance <
      ...
      showErrorAlert(
        context,
        localizations.transaction_amount_account_balance,
      );
      notifyListeners();
      return;
    }
    ...
    getIncomeAndExpenseAmount(
      genDateTimeFromMonthString(_selectedMonth, context),
    );
  ...
  /// home
  void getAllMonthsFromSignup(BuildContext context) {
  ...
  Future<void> getTransactions(BuildContext context) async {
    _transactions = await driftDb.getTransactionsByTimeFrame(
      'Month',
      dateTime: genDateTimeFromMonthString(
        _selectedMonthForTransactions,
        context,
      ),
