...
class DriftDb extends _$DriftDb {
  ...
  Future<List<TransactionModel>> getTransactionsByTimeFrame(
    ...
    switch (timeFrame) {
      ...
      case 'Week':
        start = (dateTime ?? now).subtract(Duration(days: now.weekday - 1));
        end = start
      ...
      case 'Year':
        start = DateTime(dateTime?.year ?? now.year, 1, 1);
        end = DateTime((dateTime?.year ?? now.year) + 1, 1, 1)
            .subtract(const Duration(seconds: 1));
    }
  ...
  Future<List<BudgetModel>> getAllBudgets() async {
    final budgetsList = await select(budgets).get();
    final List<BudgetModel> budgetModels = [];

    for (final budget in budgetsList) {
      final transactions = await getTransactionsByTimeFrame('Month',
          type: TransactionTypes.expense,
          categories: [stringToTransactionCategories(budget.category)]);

      final usedAmount = transactions.fold(
        0.0,
        (sum, transaction) => sum + (transaction.amount ?? 0.0),
      );

      budgetModels.add(
        BudgetModel.fromRow(budget, currentAmount: usedAmount),
      );
    }
  ...
  Future<int> createBudget(BudgetModel budget) async {
    return into(budgets).insert(
      BudgetsCompanion(
        ...
        estimatedAmount: Value(budget.estimatedAmount ?? 0),
        receiveAlert: Value(budget.receiveAlert ?? false),
        alertPercentage: Value(budget.alertPercentage ?? 0),
  ...
  Future<int> deleteBudget(int id) {
    return (delete(budgets)..where((el) => el.id.equals(id))).go();
  }

  Future<int> editBudget(BudgetModel budget) async {
    return (update(budgets)..where((el) => el.id.equals(budget.id!))).write(
      BudgetsCompanion(
        category: Value(budget.category.toString()),
        estimatedAmount: Value(budget.estimatedAmount ?? 0),
        receiveAlert: Value(budget.receiveAlert ?? false),
