...
class ReportsProvider with ChangeNotifier {
  ...
  void selectYear(String val) {
    if (val == _selectedYear) return;

    _selectedYear = val;
    _months = getOnlyMonths(genDateTimeFromYearString(val));
    _selectedMonth = null;
    getTransactionsData();
    notifyListeners();
  }
  ...
  Future<void> getTransactionsData() async {
    _expenses = await driftDb.getTransactionsByTimeFrame(
      ...
      dateTime: _selectedMonth != null
          ? genDateTimeFromMonthString('$_selectedMonth, $_selectedYear')
          : genDateTimeFromYearString(
              _selectedYear ?? DateTime.now().year.toString(),
            ),
    );
    ...
    for (Map<String, dynamic> category in categorizedExpenses) {
      _pieChartData.add(
        ChartGroupPieDataItem(
          ...
          label: getTextForTransactionCategories(category['category']),
        ),
      );
    }
    notifyListeners();
